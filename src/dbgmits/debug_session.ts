...
class DebugCommand {
   ...
   */
  constructor(cmd: string, token?: string, done?: ErrDataCallback, isNative: boolean = false) {
    this.token = token;
    this.text = cmd;
    this.done = done;
...
export default class DebugSession extends events.EventEmitter {
  ...
  private emitAsyncNotification(name: string, data: any) {
    ...
    if (event) {
      ...
      if (this.logger) {
        ...
        this.logger.warn(JSON.stringify({ name: name, data: data }) + ' Unhandled notification.');
  ...
  private parseDebbugerOutput(line: string): void {
    ...
    try {
      ...
      this.emit(Events.EVENT_DBG_CONSOLE_OUTPUT, line);
      return;
      //throw err;
    }
  ...
  private sendCommandToDebugger(command: DebugCommand): void {
    var cmdStr: string;
    if (command.isNative) {
      this.outStream.write(command.text + '\n');
      return;
    }
    if (command.token) {
  ...
  private getNativeCommandOutput<T>(command: string, token?: string, transformOutput?: (data: any) => T)
    : Promise<T> {
    return new Promise<T>((resolve, reject) => {
      this.enqueueCommand(
        new DebugCommand(command, token, (err, data) => {
  ...
  connectToRemoteTargetEx(address: string, mode: string = 'remote'): Promise<void> {
    return this.executeCommand(`target-select ${mode} ${address}`);
  }
  //
  // Breakpoint Commands
  //
   ...
   * @returns result
   */
  interpreterExec(command: string): Promise<any> {
    let fullCmd = `interpreter-exec console "${command}"`;
    return this.getCommandOutput(fullCmd, null, (output: any) => {
      return output;
    }).catch((e) => {
      logger.error(e);
   ...
   * [[EVENT_TARGET_STOPPED]] will be emitted when execution stops.
   *
   * @param pid *(GDB specific)* a process pid or a file file outside of GDB, or a thread group gid
   */
  targetAttach(pid: number | string): Promise<void> {
    var fullCmd: string = `target-attach ${pid}`;
   ...
   * @param path pathdir
   */
  environmentCd(path:  string): Promise<void> {
    var fullCmd: string = `environment-cd ${path}`;
    return this.executeCommand(fullCmd, null);
  }
